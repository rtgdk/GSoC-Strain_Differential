...
#        print "Unchanged Record: " + str(record)
        ...
        #if alt_len == 1:
            #break
        print record.samples.__len__()
        ref_num = int(record.samples[strain_pos - 1]['GT'][:1]) - 1
...
#            record.ALT[int(ref_num)] = temp
        ...
        for sample in record.samples:
            ...
            if sample.sample != strain_change_num:
                ...
                if sample['GT'] == "0|0":
                    st = str(ref_num) + "|" + str(ref_num)
#                   sample['GT'] = string
                    print sample['GT']
                    print sample
		    attr = 'GT'
                    setattr(sample, attr, st)
                elif sample['GT'] == "0/0":
                    string = str(ref_num) + "/" + str(ref_num)
                    #sample['GT'] = string
                    setattr(sample,'GT',string)
            # print sample
            # print sample['GT']
...
data = raw_input("enter data file path")
write = raw_input("enter write path")
strain = raw_input("enter strain to change reference")
#return data, write, strain
